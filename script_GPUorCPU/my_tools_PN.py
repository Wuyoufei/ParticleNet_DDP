...
def evaluate_accuracy(net, loss, data_iter,num_point_feature,GPU_mode,test=False):  
    ...
    with torch.no_grad():
        for X, y in data_iter:
            X,y=(X,y) if not GPU_mode else (X.cuda(),y.cuda())
            #note, can't use-> X,y=(X.cuda(),y.cuda()) if GPU_mode else (X,y)
...
def train_procedure_in_each_epoch(net,tran_set,loss,optimizer,num_point_feature,GPU_mode,local_rank):
