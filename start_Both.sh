...
while [ -e "./output/figs/PN_conf_${suffix}.pdf" ]
do
...
sed -i "10c #SBATCH --output=${cwd}/output/my_log/PN_${suffix}.log" run_Both.sh
sed -i "45c python -u script_GPUorCPU/my_train_DDP.py ${suffix}" run_Both.sh

if [ "$rrun" == l ]; then
    echo 'running code locally(for test usually)'
    python -u script_GPUorCPU/my_train_DDP.py ${suffix}
    exit 0
elif [ "$rrun" == s ]; then
    sbatch run_Both.sh
    exit 0
